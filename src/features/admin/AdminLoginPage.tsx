...
export const AdminLoginPage: React.FC = () => {
  ...
  const handleForgotPassword = () => {
    navigate(`/admin/recuperar-password${email ? `?email=${encodeURIComponent(email)}` : ''}`);
  };
    ...
    <div className="min-h-screen bg-(--bg-main) flex items-center justify-center p-6 font-mono transition-colors duration-300">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="space-y-6">
            <div className="space-y-2">
              <label className="text-[10px] text-gray-500 font-bold uppercase tracking-[0.2em]">Email de Acceso</label>
        ...
        <div className="mt-12 pt-8 border-t border-white/5 flex items-center justify-between opacity-30">
          ...
          <span className="text-[8px] font-bold uppercase tracking-widest">v2.5.0</span>
        </div>
      </motion.div>
    </div>
  );
